...
# Потокобезопасный генератор портов, исключающий коллизии
_port_counter = 20000
...
class SingBoxEngine:
    ...
    def _generate_config(node: ProxyNode, local_port: int) -> dict:
        ...
        # Транспорт (Поддержка xhttp / httpupgrade)
        if c.type == "ws":
    ...
    async def _run_test(self, node: ProxyNode, test_url: str, check_geo: bool = False) -> bool:
        ...
        try:
            ...
            async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
                ...
                # 1. ЖЕСТКАЯ ПРОВЕРКА СОЕДИНЕНИЯ (HTTPS Trace)
                # Это исключает ложные 200 OK от заглушек провайдеров.
                t0 = time.perf_counter()
                async with session.get("https://cloudflare.com/cdn-cgi/trace") as resp:
                    if resp.status != 200: raise Exception("Strict HTTPS check failed")
                ...
                async with session.get(test_url) as resp:
                    ...
                    if speed > 2500: speed = 999.0 # Защита от багов таймера кэша
                    node.speed = round(speed, 1)

                # 3. ГЕОЛОКАЦИЯ
                if check_geo:
                    try:
                        async with session.get(CONFIG.checking, timeout=4) as geo:
                            data = await geo.json()
    ...
    async def verify_node(self, node: ProxyNode) -> bool:
        return await self._run_test(node, CONFIG.checking, check_geo=True)
